...
def index(request):
    ...
    #follower is equal to username of currently authn user
    ...
    #displays posts one by one in the form of chain
...
@login_required(login_url='signin')
def follow(request):
    if request.method == 'POST':  nnnnnnnnn                                                                                         n    b          
...
def like_post(request):
    ...
    post_id = request.GET.get('post_id')   #GET- get the post id from url(request data from server)
...
def profile(request, pk):
    ...
    #username of currently logged in user and assigns it to follower
    user = pk
#current user (follower) is following the target user (user).
    if FollowersCount.objects.filter(follower=follower, user=user).first():
...
def settings(request):
    ...
    if request.method == 'POST':
#Checks if no new image file has been uploaded. If true, it retains the existing image
        if request.FILES.get('image') == None:
#Assigns the existing profile image to the image variable.
            ...
            user_profile.save()
#Checks if a new image file has been uploaded. Assigns the newly uploaded image to the image variable.
        if request.FILES.get('image') != None:
...
def signin(request):
    if request.method == 'POST':
        ...
        if user is not None:  #checks if un and pw was correct(authn was successful)
...
def send_email_notification(request, uploaded_by):
    ...
    message = f'Hello,\n\n{uploaded_by.username} has uploaded a new photo on the platform.\n\n' \
              f'You can check it out on the platform.'
